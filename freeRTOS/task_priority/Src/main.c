...
int main(void)
{
	...
	//Start Recording
	// this is for the SEGGER view so you can look at the trace to debug easier
	SEGGER_SYSVIEW_Conf();
...
void vTask2_handler(void *params)
{
	...
	while(1)
	{
		if( switch_prio )	// if the button is pressed, execute below
		{
			switch_prio = FALSE;
			...
			//switch prio
			// switch the priority here
			vTaskPrioritySet(xTaskHandle1,p2);
			vTaskPrioritySet(xTaskHandle2,p1);	// decrease the priority of task2 after the first iteration


		}else
		{	// if the button isn't pressed, do this
			GPIO_ToggleBits(GPIOA,GPIO_Pin_5);
			rtos_delay(1000);
		}
...
void EXTI15_10_IRQHandler(void)
{
	...
	switch_prio = TRUE;	// if it runs, sets the flag to true
	traceISR_EXIT();
